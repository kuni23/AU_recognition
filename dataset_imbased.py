...
class AUDatasetPrototype(Dataset):
    ...
    def __init__(self, db_path : str, dataset_name : str, data_type : str, list_of_data_needed : Dict[str, str],
                annotation_type = 'AU', way_of_split = 1.0, transform = None, annot_indices=None):

        self.name_tag = Path(db_path).stem + '-' + dataset_name
        self.AUDM = AuDataManagementPrototype(db_path, dataset_name, data_type, annotation_type, way_of_split, annot_indices, list_of_data_needed, transform)
    ...
    def __getitem__(self, idx):
        ...
        current_input, y = self.AUDM.load_data_to_memory(idx)
    ...
    def __len__(self):
        return self.AUDM.dataset_length

    def get_dataid_refpos(self, indices):
        data_ids, ref_poses = [], []
        for idx in indices:
            data_id, ref_pos = self.AUDM.get_dataid_refpos(idx)
            data_ids.append(data_id)
            ref_poses.append(ref_pos)
        return data_ids, ref_poses
...
class EmotionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.annotation_type == 'emotion':
            current_emotions = [y[str(emotion)] for emotion in EM_INDICES]
            current_emotions = np.array(current_emotions, dtype=np.float32)
